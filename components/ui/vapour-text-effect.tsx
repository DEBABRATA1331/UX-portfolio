...
const createParticles = (
    ...
    // For very large fonts, we want to aggressively sample fewer pixels to maintain 60FPS
    const sizeMultiplier = metrics.width > 500 ? 3 : 1;
    const baseSampleRate = Math.max(2, Math.round((currentDPR / baseDPR) * sizeMultiplier));
...
const calculateVaporizeSpread = (fontSize: number | string) => {
    const size = typeof fontSize === "string" ? parseInt(fontSize.replace("px", "")) : fontSize;
